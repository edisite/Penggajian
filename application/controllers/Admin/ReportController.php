...
class ReportController extends CI_Controller
{
	public function __construct()
	{
		...
		{
			$config = $this->config->item('hakakses');
	...
	public function index()
	{
		if ($this->session->userdata('role') === '1') {
			...
			$data['absensi']	 	= $this->db->query("SELECT * FROM absensi INNER JOIN karyawan ON absensi.id_karyawan = karyawan.id INNER JOIN jabatan ON karyawan.id_jabatan = jabatan.id_jabatan INNER JOIN bagian ON jabatan.id_bagian = bagian.id_bagian  and 
			absensi.periode_tanggal= '".$datefil."'")->result();
	...
	public function print_absensi($datetrans = '')
	{
		$date = date_create($datetrans);
		$dateformat = date_format($date, 'F Y');

		$sql = "SELECT * FROM absensi INNER JOIN karyawan ON absensi.id_karyawan = karyawan.id INNER JOIN jabatan ON karyawan.id_jabatan = jabatan.id_jabatan INNER JOIN bagian ON jabatan.id_bagian = bagian.id_bagian 
		and absensi.periode_tanggal= '$datetrans'";
		...
		$pdf->WriteHTMLCell(0, 0, '', '', '<p>LAPORAN BULAN : '.strtoupper($dateformat).'</p>', 0, 1, 0, true, 'C', true);
		...
		// 	";
			...
			<td style="border:1px solid #ddd;" colspan="9">Record Empty</td>
			</tr>';
		}
		$table .= '</table>';
	...
	public function gaji_bersih()
	{
		if ($this->session->userdata('role') === '1') {
			...
			$data['penggajian']	 	= $this->db->query("SELECT * FROM penggajian INNER JOIN karyawan ON penggajian.id_karyawan = karyawan.id INNER JOIN jabatan ON karyawan.id_jabatan = jabatan.id_jabatan INNER JOIN bagian ON jabatan.id_bagian = bagian.id_bagian and penggajian.periode_bulan ='".$datefil."'")->result();
	...
	public function print_gaji_bersih($datefil = '')
	{
		$date = date_create($datefil);
		$dateformat = date_format($date, 'F Y');

		$data	 			= $this->db->query("SELECT * FROM penggajian INNER JOIN karyawan ON penggajian.id_karyawan = karyawan.id INNER JOIN jabatan ON karyawan.id_jabatan = jabatan.id_jabatan INNER JOIN bagian ON jabatan.id_bagian = bagian.id_bagian and penggajian.periode_bulan ='".$datefil."'")->result();
		...
		if (!empty($data)) {
			...
			// ";
			$table .= '<tr align="center">
			<td style="border:1px solid #ddd;" colspan="6">Record Empty</td>
			</tr>';
		}
		$table .= '</table>';
		$pdf->WriteHTMLCell(0, 0, 15, '', $table, 0, 1, 0, true, 'C', true);
