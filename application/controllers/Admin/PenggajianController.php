	...
	public function update()
	{
		...
		if ($result->num_rows() > 0) {
			...
			$id_jabatan		= $data['id_jabatan'];
                        ...
                        $periode    = $this->db->query("SELECT * FROM penggajian WHERE id_penggajian = $id");
                        $periode_bulan = '';
			if ($periode->num_rows() > 0) {
				$data			= $periode->row_array();
			...
			$result2						= $this->db->query("SELECT * FROM absensi WHERE id_karyawan = '".$karyawan."' AND periode_tanggal = '".$periode_bulan."'");
